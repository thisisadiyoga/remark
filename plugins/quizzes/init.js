...
(function() {
  ...
  */
  ...
  function QUIZ$POLL(poll, qrimg, _elems, kind) {
    if(poll && poll.length > 0) {
      if(qrimg && qrimg.length > 0) {
        ...
        const _chld = qrimg[0].childNodes[0];
        if (typeof _chld === 'string') {
          _qr.src = _chld;
        } else if (_chld.tagName === 'A') {
          _qr.src = _chld.innerHTML;
        }
  ...
  function init(opt, slideshow) {
    ...
    if (opt && opt.qanvas && opt.qanvas.date) {
      ANS_DATE = new Date(opt.qanvas.date);
