(function() {
  /* Options
  toc: {
    multiTitle: false / true (default: false)
    unique: false / true     (default: false)
    title: 'titles'
    section: 'sections'
  }
  */
  ...
  function initTOCElem_content(opt, slideshow) {
    const isSingle = opt && opt.toc && opt.toc.multiTitle && opt.toc.multiTitle;
    if(isSingle) {
      ...
      function initTOCElem_content_single(opt, slideshow) {
        ...
        const titleClass = (opt && opt.toc && opt.toc.title) ?? TITLE;
        const sectionClass = (opt && opt.toc && opt.toc.section) ?? SECTIONS;
        
        /* Titles */
        let title = document.getElementsByClassName(titleClass)[0];
        let lnk, nm, index;
        ...
        /* Subtitles */
        const subtitles = document.getElementsByClassName(sectionClass);
        const subttl = {};
      ...
      function initTOCElem_content_multi(opt, slideshow) {
        ...
        const titleClass = (opt && opt.toc && opt.toc.title) ?? TITLE;
        const sectionClass = (opt && opt.toc && opt.toc.section) ?? SECTIONS;
        const alltitles = document.querySelectorAll(`.${titleClass},.${sectionClass}`);
        const ttl = {};
        const sub = {};
        
        for(const title of alltitles) {
          let isTitle = hasKlass(title.classList, titleClass);
